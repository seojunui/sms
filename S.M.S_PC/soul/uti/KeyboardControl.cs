...
namespace soul.uti
{
    class KeyboardControl
    {
        private Dictionary<string, byte> keypacket = new Dictionary<string, byte>()
        {
            ...
            {"Alt",0X12},{"kor",0X15},{"chn",0X19},{"Esc",0X1B},{"space",0X20},{"PgUp",0X21},
            ...
            {"F7",0X76},{"F8",0X77},{"F9",0X78},{"F10",0X79},{"F11",0X7A},{"F12",0X7B} ,{"CapsLock",0x14} ,{",",0xBC} ,{"`",0xC0},
            //{"/",0xBF},{"[",0xDB},{"]",0xDD},{";",0xBA},{"'",0xDE},{"window",0x5B},{"=",187}
        ...
        public void Start(string keyValue , Dictionary<String,byte> keyPack)
        {
            try
            {
                Console.WriteLine(keyValue);
                if (keyPack.ContainsKey(keyValue))
                {
                    if (System.Windows.Forms.Control.IsKeyLocked(System.Windows.Forms.Keys.CapsLock))
                    {
                        keybd_event(keypacket["CapsLock"], 0, 0, 0);
                    ...
                    keybd_event(keyPack[keyValue], 0, 0, 0);
                    keybd_event(keyPack[keyValue], 0, 0x0002, 0);
                }
                else
                {
                    string resultHex = string.Empty;
                    ...
                    if (arr_byteStr[0] >= 65 && arr_byteStr[0] <= 90)
                    {
                        if (!System.Windows.Forms.Control.IsKeyLocked(System.Windows.Forms.Keys.CapsLock))
                        {
                            keybd_event(keyPack["CapsLock"], 0, 0, 0);
                            keybd_event(keyPack["CapsLock"], 0, 0x0002, 0);
                        }
